...
def index():
    db, c = get_db()
    c.execute(
        'SELECT t.id, t.description, u.username, t.completed, t.created_at FROM todo t JOIN users u on t.created_by = u.id WHERE t.created_by = %s'
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'SELECT t.id, t.description, t.completed, t.created_by, t.created_at, u.username FROM todo t JOIN users u ON t.created_by = u.id WHERE t.id = %s', (
            id,)
    )
